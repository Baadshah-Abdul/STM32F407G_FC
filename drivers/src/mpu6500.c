		...
		float *off_gyro)
{
	...
	mpu_data.accel_f[0] = (mpu_data.accel_f[0] * (1.0f - ACCEL_ALPHA))
			+ (ax_now * ACCEL_ALPHA);
	mpu_data.accel_f[1] = (mpu_data.accel_f[1] * (1.0f - ACCEL_ALPHA))
			+ (ay_now * ACCEL_ALPHA);
	mpu_data.accel_f[2] = (mpu_data.accel_f[2] * (1.0f - ACCEL_ALPHA))
			+ (az_now * ACCEL_ALPHA);

	mpu_data.gyro_f[0] = (mpu_data.gyro_f[0] * (1.0f - GYRO_ALPHA))
			+ (gx_now * GYRO_ALPHA);
	mpu_data.gyro_f[1] = (mpu_data.gyro_f[1] * (1.0f - GYRO_ALPHA))
			+ (gy_now * GYRO_ALPHA);
	mpu_data.gyro_f[2] = (mpu_data.gyro_f[2] * (1.0f - GYRO_ALPHA))
			+ (gz_now * GYRO_ALPHA);
}
...
void MPU6500_CalculateAngles(void)
{
	...
	float acc_roll = atan2f(mpu_data.accel_f[1],
			mpu_data.accel_f[2]) * RAD_TO_DEG;
	float acc_pitch = atan2f(-mpu_data.accel_f[0],
			sqrtf(
					mpu_data.accel_f[1] * mpu_data.accel_f[1]
							+ mpu_data.accel_f[2]
									* mpu_data.accel_f[2])) * RAD_TO_DEG;
